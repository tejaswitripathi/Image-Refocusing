...
# ------------------
# S3 checkpoint config (matches trainSharpenerNet.py)
# ------------------

S3_BUCKET = "tejas-blender-bucket"
S3_CHECKPOINT_PREFIX = "defocus-checkpoints/sharpener-net"
S3_BEST_KEY = f"{S3_CHECKPOINT_PREFIX}/best_sharpener.pth"
...
def display_defocused_recreated_target(defocused, recreated, target, coc_px=None):
    ...
    # CoC heatmap underneath the NN sharp panel.
...
def recreate_sharp_with_model(datadir, device=None):
    if device is None:
    ...
    defocused_chw = np.transpose(defocused_rs, (2, 0, 1))  # [3, H, W]
    ...
    # ----------------------------
    # Input tensor: defocused RGB (3) + f-stop (1) + focal (1) + CoC (1) = 6 ch
    # ----------------------------

    x = np.concatenate(
        [defocused_chw, fstop_map, focal_map, coc_channel],
        axis=0
    ).astype(np.float32)
    ...
    return defocused, pred, coc_px_display
...
gt_defocused, nn_sharp, coc_px = recreate_sharp_with_model(datadir)
...
# ----------------------------
# Error metrics (NN sharp vs. ground-truth sharp)
# ----------------------------

mse = mean_squared_error(
    np.clip(gt_sharp, 0, 1),
    np.clip(nn_sharp, 0, 1)
)
psnr, ssim = evaluate_metrics(nn_sharp, gt_sharp)
...
display_defocused_recreated_target(gt_defocused, nn_sharp, gt_sharp, coc_px)
